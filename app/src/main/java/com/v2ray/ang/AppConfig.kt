...
object AppConfig {
    ...
    // V2Board API Configuration for 云通 (yuntong.org)
    const val V2BOARD_BASE_URL = "https://yuntong.org"
    const val V2BOARD_API_URL = "$V2BOARD_BASE_URL/api/v1"
    const val V2BOARD_SUBSCRIBE_URL = "$V2BOARD_API_URL/user/server/fetch"
    
    const val ANDROID_PACKAGE_NAME_LIST_URL = "$GITHUB_RAW_URL/2dust/androidpackagenamelist/master/proxy.txt"
    const val APP_URL = "https://yuntong.org"
    const val APP_API_URL = ""  // Disabled - using V2Board API
    const val APP_ISSUES_URL = "https://t.me/yuntong_vpn"
    const val APP_WIKI_MODE = "https://yuntong.org"
    const val APP_PRIVACY_POLICY = "https://yuntong.org"
    const val APP_PROMOTION_URL = ""
    const val TG_CHANNEL_URL = "https://t.me/yuntong_vpn"
    ...
    const val RAY_NG_CHANNEL_NAME = "云通 后台服务"
